...
| `ptah_create_ticket` | Create a new ticket in an existing project. Accepts `type` (`task` or `epic`) and `parent` (the id of the epic it sits under — must be an epic, may be in another project; only a task can have one). |
| `ptah_update_ticket` | Patch fields on an existing ticket. Omitted fields are left unchanged; passing `due: null` clears the due date, and `parent: null` detaches a task from its epic. |
...
Parent/child nesting is exactly two levels deep: only an epic can hold sub-tasks, and only
a task can be one. An epic cannot have a parent, and a task cannot have sub-tasks.
